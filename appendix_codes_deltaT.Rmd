...
# Changing number of timesteps(50 - 8000): Setting up the bounds
```{r}
seed <- 20230331
T <- 5
sigma <- 0.25
mu <- 0.01
repeat.count <- 500
N.list <- c(50, 125, 250, 500, 1000, 2000, 4000, 8000)
df.err <- data.frame(mu=numeric(0), method=character(0), 
                     err=numeric(0), abs.err=numeric(0))
```

# Changing number of timesteps(50 - 8000): Monte-Carlo Simulation of Black-Scholes True Solution
```{r}
sim.true <- numeric(0)
set.seed(seed)
for (N in N.list) {
...
# Changing number of timesteps(50 - 8000): Monte-Carlo Simulation of Milstein
```{r}
sim.mil <- numeric(0)
dummy.N <- numeric(0)
set.seed(seed)
for (N in N.list) {
  ...
  dummy.N <- c(dummy.N, rep(N, repeat.count*(N+1)))
}
df.err.mil <- data.frame(N=dummy.N,
               method=rep("mil",repeat.count*(sum(N.list)+length(N.list))),
               err=sim.mil-sim.true, abs.err=abs(sim.mil-sim.true))
```

# Changing number of timesteps(50 - 8000): Monte-Carlo Simulation of Second Milstein Scheme
```{r}
sim.mil2 <- numeric(0)
dummy.N <- numeric(0)
set.seed(seed)
...
df.err.mil2 <- data.frame(N=dummy.N,
               method=rep("mil2",repeat.count*(sum(N.list)+length(N.list))),
               err=sim.mil2-sim.true, abs.err=abs(sim.mil2-sim.true))
```

# Changing number of timesteps(50 - 8000): Monte-Carlo Simulation of Runge-Kutta
```{r}
sim.rk <- numeric(0)
dummy.N <- numeric(0)
set.seed(seed)
...
df.err.rk <- data.frame(N=dummy.N,
               method=rep("rk",repeat.count*(sum(N.list)+length(N.list))),
               err=sim.rk-sim.true, abs.err=abs(sim.rk-sim.true))
```

# Changing number of timesteps(50 - 8000): Mean and Confident Interval of Absolute Errors
```{r}
conf.int.mil <- aggregate(df.err.mil$abs.err, list(df.err.mil$N), 
                        FUN=quantile, probs=c(0.025, 0.975))
df.err.mil2 <- df.err.mil2[is.finite(df.err.mil2$abs.err),]
conf.int.mil2 <- aggregate(df.err.mil2$abs.err, list(df.err.mil2$N), 
                        FUN=quantile, probs=c(0.025, 0.975))
conf.int.rk <- aggregate(df.err.rk$abs.err, list(df.err.rk$N), 
                        FUN=quantile, probs=c(0.025, 0.975))
mean.mil <- aggregate(df.err.mil$abs.err, list(df.err.mil$N), FUN=mean)
mean.mil2 <- aggregate(df.err.mil2$abs.err, list(df.err.mil2$N), FUN=mean)
mean.rk <- aggregate(df.err.rk$abs.err, list(df.err.rk$N), FUN=mean)
...
# Changing number of timesteps(50 - 8000): Plot the results
```{r warning=FALSE}
df.err <- rbind(df.err.mil, df.err.mil2, df.err.rk)
df.err$N <- factor(df.err$N)
p <-df.err %>%
  mutate(  ) %>%
  ggplot( aes(x=N, y=abs.err, fill=method) ) +
    geom_boxplot(outlier.shape=NA) +
    ggtitle("Varying Number of Timesteps (50 - 8000)") +
    ...
    scale_x_discrete(labels = N.list)+
