...
for (mu in mu.list) {
  ...
  df.mil = data.frame(mu=rep(mu,N+1),method=rep("mil",N+1),err=abs(sim.mil-sim.true))
  df.mil2 = data.frame(mu=rep(mu,N+1),method=rep("mil2",N+1),err=abs(sim.mil2-sim.true))
  df.euler = data.frame(mu=rep(mu,N+1),method=rep("euler",N+1),err=abs(sim.euler-sim.true))
  df.rk = data.frame(mu=rep(mu,N+1),method=rep("rk",N+1),err=abs(sim.rk-sim.true))
...
p <-df.err %>%
  mutate( bin=cut_width(mu, width=mu.delta, boundary=0) ) %>%
