...
delta.list <- seq(delta.lower, delta.upper, delta.delta)

df.err = data.frame(mu=numeric(0), method=character(0), err=numeric(0))

for (del in delta.list) {
  d <- parse(text = paste(mu,"* x"))
  s <- parse(text = paste(sigma,"* x"))
  
  N = T/del

  sim.true <- sde.bs.true(T=T, delta.t=del, seed=seed, theta=c(mu,sigma))
  sim.rk <- sde.rk(T=T, delta.t = del, seed=seed, theta=c(mu,sigma))
  set.seed(seed)
  sim.mil <- as.numeric(sde.sim(X0=100, drift=d, sigma=s,
                     method="milstein", N=N, T=T, delta = del))
  set.seed(seed)
  sim.mil2 <- as.numeric(sde.sim(X0=100, drift=d, sigma=s, 
                      method="milstein2", N=N, T=T, delta = del))
  set.seed(seed)
  sim.euler <- as.numeric(sde.sim(X0=100, drift=d, sigma=s,
                       method="euler", N=N, T=T, delta = del))
  
  df.mil = data.frame(delta=rep(del,N+1),method=rep("mil",N+1),err=abs(sim.mil-sim.true))
  df.mil2 = data.frame(delta=rep(del,N+1),method=rep("mil2",N+1),err=abs(sim.mil2-sim.true))
  df.euler = data.frame(delta=rep(del,N+1),method=rep("euler",N+1),err=abs(sim.euler-sim.true))
  df.rk = data.frame(delta=rep(del,N+1),method=rep("rk",N+1),err=abs(sim.rk-sim.true))
...
p <-df.err %>%
  mutate( bin=cut_width(delta, width=delta.delta, boundary=0) ) %>%
  ggplot( aes(x=bin, y=err, fill=method) ) +
    ...
    scale_x_discrete(labels = delta.list) +
    ggtitle("Varying Time Step Size (0.01 and above)")
